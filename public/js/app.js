...
async function fetchState() {
  try {
    ...
    console.warn('Backend API unavailable, loading local fallback seed data', err);
    // Fallback if accessed via direct file preview
...
function renderActiveMeeting(index) {
  ...
  // Live Extracted Intelligence Cards
...
function renderKnowledgeGraphTab() {
  // Construct dynamic multi-meeting knowledge nodes & edges
  const nodes = [
...
function renderRisksAndContradictions() {
  ...
  if (contraContainer && state.contradictions) {
    let html = '';
...
async function handleWhyQuery(customQuestion) {
  ...
  try {
    ...
    console.warn('Falling back to local query engine:', err);
    // Local query evaluation fallback
    renderAnswerLocal(question);
  }
}
...
function renderAnswerLocal(question) {
  const q = question.toLowerCase();
  let fakeData = {
    headline: "PostgreSQL chosen for ACID compliance & team experience.",
    details: {
      decision: "Adopt PostgreSQL for Project V2",
      meeting: "Architecture Review (July 18, 2026)",
      rationale: "Project V2 demands strict ACID transaction compliance for billing engine and team has 5+ years expertise.",
      alternatives: ["MongoDB", "MySQL"],
      decisionMakers: "Sarah & Elena",
      evidenceQuote: "Sarah: 'We evaluated MongoDB, MySQL, and PostgreSQL. PostgreSQL is our recommended choice...'"
    },
    graphBreadcrumbs: [
      { label: 'Project V2', type: 'Project' },
      { label: 'PostgreSQL', type: 'Tech' },
      { label: 'Architecture Review', type: 'Meeting' }
    ]
  };
  renderAnswer(fakeData);
}
